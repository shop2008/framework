...
public class MessageManagementServiceImpl extends AbstractModule<IStockAppContext> implements IMessageManagementService {
	...
	public List<PullMessageBean> getUnReadPullMessage() {
		List<PullMessageBean> msgs =  queryRemindMessages();
	...
	public List<RemindMessageBean> getUnReadRemindMessage() {
		List<RemindMessageInfo> msgs = getService(IDBService.class).getDaoSession().getRemindMessageInfoDao().queryRaw(" where READ=0 and USER_ID=?",getUserId());
		if (log.isDebugEnabled()) {
	...
	public void savePullMsg(PullMessageBean msg) {
		...
		List<PullMessageInfo> list=dao.queryRaw("where PULL_ID =? and USER_ID =?", String.valueOf(msg.getId()),getUserId());
