...
public class GMinuteLineView extends GBasicLineView  implements IDataChangedListener  {
	
	private static Trace log = Trace.getLogger(GMinuteLineView.class);
	...
	public void onDraw(Canvas canvas) {
		...
		onDrawHighPriceText(canvas);
		onDrawLowPriceText(canvas);
		onDrawHighPercentageText(canvas);
		onDrawLowPercentageText(canvas);
		/** 设置分时线表格左边的股票价格文字 */
...
//		setOnDrawSecuVolumeChart(canvas);
	...
	private void onDrawHighPriceText(Canvas canvas){
		canvas.save();
		canvas.restore();
		...
		//画黑色背景
		int w = getRectDrawTextWidth(mPaint, tempHighPice) + 6;
		canvas.drawRect(fStartX, fStartY-minuteTopPadding, w + fStartX, 40, getBackgroundPaint());
		
		//画边框
		canvas.drawRect(fStartX, fStartY-minuteTopPadding, w + fStartX, 40, getBorderPaint());
		
		//画文字
		canvas.drawText(tempHighPice, fStartX+w-3, fStartY + 13, mPaint);
		canvas.restore();
	} 
	private void onDrawLowPriceText(Canvas canvas){
		canvas.save();
		mPaint.setAntiAlias(true);
		canvas.setDrawFilter(new PaintFlagsDrawFilter(0, Paint.ANTI_ALIAS_FLAG|Paint.FILTER_BITMAP_FLAG));  
		...
		//画黑色背景
		int w = getRectDrawTextWidth(mPaint, templowPrice) + 6;
		canvas.drawRect(fStartX, fStartY+lineHeight*4+minuteBottomPadding-30, w + fStartX, fStartY+lineHeight*4+minuteBottomPadding, getBackgroundPaint());
		
		//画边框
		canvas.drawRect(fStartX, fStartY+lineHeight*4+minuteBottomPadding-30, w + fStartX, fStartY+lineHeight*4+minuteBottomPadding, getBorderPaint());
		
		//画文字
		canvas.drawText(templowPrice, fStartX+w-3, fStartY + lineHeight*4, mPaint);
		canvas.restore();		
	} 
	private void onDrawHighPercentageText(Canvas canvas){
		canvas.save();
		canvas.restore();
		mPaint.setAntiAlias(true);
		mPaint.setTextSize(textSize);
		...
		if(highPrice>0 && yesterdayClose>0){
			highPicePercentage = String.format("%.2f%%", (highPrice - yesterdayClose)/yesterdayClose*100.00);
		}	
		//画黑色背景
		int w = getRectDrawTextWidth(mPaint, highPicePercentage) + 6;
		canvas.drawRect(fStopX - w -1, fStartY-minuteTopPadding, fStopX-1, 40, getBackgroundPaint());
		
		//画边框
		canvas.drawRect(fStopX - w, fStartY-minuteTopPadding, fStopX, 40, getBorderPaint());
		
		//画文字
		canvas.drawText(highPicePercentage, fStopX - w + 3, fStartY + 13, mPaint);
		canvas.restore();
	}
	private void onDrawLowPercentageText(Canvas canvas){
		canvas.save();
		canvas.restore();
		mPaint.setAntiAlias(true);
		mPaint.setTextSize(textSize);
		...
		if(lowPrice>0 && yesterdayClose>0){
			lowPicePercentage = String.format("%.2f%%", (yesterdayClose - lowPrice)/yesterdayClose*100.00);
		}
		int w = getRectDrawTextWidth(mPaint, lowPicePercentage) + 6;
		//画黑色背景
		canvas.drawRect(fStopX - w -1, fStartY+lineHeight*4+minuteBottomPadding-30, fStopX-1, fStartY+lineHeight*4+minuteBottomPadding, getBackgroundPaint());
		//画边框
		canvas.drawRect(fStopX - w, fStartY+lineHeight*4+minuteBottomPadding-30, fStopX, fStartY+lineHeight*4+minuteBottomPadding, getBorderPaint());
		//画文字
		canvas.drawText(lowPicePercentage, fStopX - w + 3, fStartY + lineHeight*4, mPaint);
		canvas.restore();
	}
	...
	private Paint getBorderPaint(){
		...
		bPaint.setColor(getBorderColor());
	...
	//精确计算文字宽度
	private static int getDrawTextWidth(Paint paint, String str){
		int iRet = 0;
		if(str!=null && str.length()>0 && StringUtils.isNotEmpty(str)){
			int len = str.length();
...
//		mPaint.setColor(getStockUpColor());// 上涨
//		String tempHighPice = String.format("%.2f", highPrice/1000);	
//		canvas.drawText(tempHighPice, fStartX+50, fStartY + padding + 6, mPaint);
//		
//		mPaint.setPathEffect(null);
//		mPaint.setTextSize(18);
//		mPaint.setColor(getStockDownColor()); //下跌
//		String templowPrice = String.format("%.2f", lowPrice/1000);
//		canvas.drawText(templowPrice, fStartX+50, fStartY + padding + lineHeight*4-6, mPaint);		
//		canvas.restore();
//	}
...
//		float padding = 5;
//		float textMarginRight = fStopX - 65;
//		mPaint.setAntiAlias(true);
//		canvas.setDrawFilter(new PaintFlagsDrawFilter(0, Paint.ANTI_ALIAS_FLAG|Paint.FILTER_BITMAP_FLAG));  
//		mPaint.setTextAlign(Paint.Align.LEFT);
//		mPaint.setColor(getStockUpColor());
...
//			canvas.drawText(highPicePercentage, textMarginRight+3, fStartY + padding+6, mPaint);
//		}else{
//			canvas.drawText("0.00%", textMarginRight+6, fStartY+6 + padding, mPaint);
//		}
//		mPaint.setColor(getStockDownColor());
//		if(lowPrice>0 && yesterdayClose>0){
