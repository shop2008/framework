 ...
 * 资讯&公告界面自定义
 */
public class InfoNoticesView extends PinnedHeaderListView {

	private PinHeaderListAdapter pinHeaderAdapter;
	private int mLocationPosition = -1;

	public InfoNoticesView(Context context) {
		super(context);
	}

	public InfoNoticesView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public InfoNoticesView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	/** Adapter ------- begin */
	private class PinHeaderListAdapter extends BaseAdapter implements
			OnScrollListener, SectionIndexer, PinnedHeaderAdapter {
				...
				for (int i = 0; i < provider.getItemCounts(); i++) {
					...
					if (object instanceof MessageInfoBean) {
						...
						if (i == 0) {
							...
							if (messageInfoBean.getDate() != curMessage
									.getDate()) {
		...
		private int compare(String dateStr1, String dateStr2)
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = inflater.inflate(
						R.layout.user_notice_info_item_layout, null);
			}
			LinearLayout mHeaderParent = (LinearLayout) convertView
					.findViewById(R.id.notice_header_container);
			TextView mHeaderText = (TextView) convertView
					.findViewById(R.id.notice_data_label);
			...
			if (object != null && object instanceof MessageInfoBean) {
				...
				TextView news_date = (TextView) convertView
						.findViewById(R.id.notice_date);
				TextView news_content = (TextView) convertView
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			if (view instanceof InfoNoticesView) {
				((InfoNoticesView) view).configureHeaderView(firstVisibleItem);
			}
		}
	...
	protected void updateUI() {
		...
		setPinnedHeaderView(LayoutInflater.from(getContext()).inflate(
				R.layout.header_layout, this, false));
	}
}
