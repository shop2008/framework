 ...
 * 
 */
@View(name = "otherUserSellOutPage", withToolbar=true, description="参赛交易盘")
@AndroidBinding(type = AndroidBindingType.FRAGMENT_ACTIVITY, layoutId = "R.layout.other_user_sell_out_layout")
public abstract class OtherUserSellOutPage extends PageBase implements
		IModelUpdater {
	...
	@Bean(type = BindingType.Pojo, express = "${tradingService!=null?tradingService.getTradingAccountInfo(accId):null}")
	...
	/** 总盈亏率 */
	@Field(valueKey = "text", binding = "${tradingAccount!=null?tradingAccount.gainRate:0}", attributes = { @Attribute(name = "textColor", value = "${(tradingAccount!=null && tradingAccount.gainRate>0)?'resourceId:color/red':((tradingAccount!=null && tradingAccount.gainRate<0)?'resourceId:color/green':'resourceId:color/white')}") }, converter = "stockL2StrConvertor")
	String gainRate;

	/** 总盈亏额 */
	@Field(valueKey = "text", binding = "${tradingAccount!=null?tradingAccount.totalGain:0}", attributes = { @Attribute(name = "textColor", value = "${(tradingAccount!=null && tradingAccount.totalGain>0)?'resourceId:color/red':((tradingAccount!=null && tradingAccount.totalGain<0)?'resourceId:color/green':'resourceId:color/white')}") }, converter = "float2StringConvertor")
	String totalGain;
	...
	CommandResult toolbarClickedRight(InputEvent event) {
		...
		if (event.getProperty("position") instanceof Integer) {
			...
			if (stockTradingOrder != null && stockTradingOrder.size() > 0) {
				StockTradingOrderBean tempTradingA = stockTradingOrder
						.get(position);
	...
	public void updateModel(Object value) {
		...
		if (isVirtual != null) {
			/*IAppToolbar toolbar = getPageToolbar();
			if (toolbar != null) {
				if (isVirtual) {
