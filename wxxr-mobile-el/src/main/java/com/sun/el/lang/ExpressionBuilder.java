 ...
 */
public final class ExpressionBuilder extends AbstractNodeVisitor {
...
//            processAstFunctiom((AstFunction)node);
//        } else if (node instanceof AstIdentifier) {
//            processAstIdentifier((AstIdentifier)node);
//        }
//    }
	 ...
	 * @param node
	 */
	protected void processAstIdentifier(AstIdentifier node) {
	 ...
	 * @param node
	 */
	protected void processAstFunctiom(AstFunction funcNode) {
		...
		int acnt = ((AstMethodArguments)funcNode.jjtGetChild(0)).getParameterCount();
		if (acnt != pcnt) {
		    throw new ELException(MessageFactory.get(
	...
	public void visit(AstIdentifier node) {
		 processAstIdentifier(node);
	}

	/* (non-Javadoc)
	 * @see com.sun.el.parser.AbstractNodeVisitor#visit(com.sun.el.parser.AstFunction)
	 */
	@Override
	public void visit(AstFunction node) {
		processAstFunctiom(node);
	}
}
