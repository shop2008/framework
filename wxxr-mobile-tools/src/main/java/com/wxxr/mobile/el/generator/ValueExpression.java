   ...
   public void generateJavaCode(CompilingContext ctx,StringBuffer buf){
      ...
      if((returnType.getKind() == TypeKind.VOID)&&(returnType.getKind() == TypeKind.NONE)){
         if(this.varAssignedInBody){
            buf.append(getType().toString()).append(' ').append(this.varName).append(" = ").append(ctx.getResolver().getDefaultValueOfType(getType())).append(";\n");
         }else{
            buf.append(getType().toString()).append(' ').append(this.varName).append(" = ");
         }
      }
