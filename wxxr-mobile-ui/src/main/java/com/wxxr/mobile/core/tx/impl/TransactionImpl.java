...
import com.wxxr.javax.transaction.HeuristicMixedException;
import com.wxxr.javax.transaction.HeuristicRollbackException;
import com.wxxr.javax.transaction.RollbackException;
import com.wxxr.javax.transaction.Status;
import com.wxxr.javax.transaction.Synchronization;
import com.wxxr.javax.transaction.SystemException;
import com.wxxr.javax.transaction.Transaction;
import com.wxxr.javax.transaction.XAException;
   ...
   implements Transaction
{
   ...
   private static Trace log = Trace.getLogger(TransactionImpl.class);
...
//   static TimeoutFactory timeoutFactory = TimeoutFactory.getSingleton();
   ...
   TransactionImpl(long timeout)
   {
      ...
      this.timeout = createTimeout(timeout);
	 ...
	 * @return
	 */
	protected ICancellable createTimeout(long timeout) {
      ...
      try
      {
         ...
         if (status == Status.STATUS_ACTIVE)
         {
            ...
            case 1:
               ...
               break;
            default:
            	throw new SystemException("Two phases commit is not supported !");
//               // Two phase commit
//               if (trace)
...
//                  status = Status.STATUS_COMMITTED; // all was read-only
         ...
         if (status != Status.STATUS_COMMITTED)
         {
            ...
            throw new RollbackException("Unable to commit, tx=" +
   ...
   int prepare() throws HeuristicMixedException, HeuristicRollbackException, RollbackException
   {
      ...
      try
      {
         ...
         if (status != Status.STATUS_PREPARED)
         {
            ...
            // throw jboss rollback exception with the saved off cause
            throw new RollbackException("Unable to prepare, tx=" +
                  toString() + " status=" + getStringStatus(status),
                  causedByThrowable);
         }
    ...
    */
   void setWork(Object work) throws SystemException
   {
      lock();
      try
      {
         ...
         if (status == Status.STATUS_NO_TRANSACTION || status == Status.STATUS_UNKNOWN)
            throw new SystemException("The transaction is not active " + this + ": " + getStringStatus(status));
         else if (status != Status.STATUS_ACTIVE)
            throw new SystemException("Too late to start work " + this + ": " + getStringStatus(status));
         else if (this.work != null)
            throw new SystemException("Already have work " + this + ": " + this.work);
